...
import { BuscadorModuloRecurso } from 'app/recurso/programa/BuscadorModuloRecurso';

@Injectable()
export class BuscadorModuloService extends BaseServicio {

  constructor(private buscadorModuloRecurso: BuscadorModuloRecurso) {
    super(buscadorModuloRecurso);
  }
  ...
  getDatosInicio(): Promise<any> {
    if (this.datosInicio) {
      ...
      var resultado = this.buscadorModuloRecurso.getDatosInicio();
      this.promesaDatosInicio = new Promise((resolve, reject) => {
        resultado
          .then(
            (res: any) => {

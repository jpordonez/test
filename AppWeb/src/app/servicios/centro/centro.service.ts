...
import { CentroRecurso } from 'app/recurso/centro/CentroRecurso';

@Injectable()
export class CentroService extends BaseServicio {

  constructor(private centroRecurso: CentroRecurso) {
    super(centroRecurso);
  }
  ...
  getDatosInicio(): Promise<any> {
    if (this.datosInicio) {
      ...
      var resultado = this.centroRecurso.getDatosInicio();
      this.promesaDatosInicio = new Promise((resolve, reject) => {
        resultado
          .then(
            (res: any) => {
